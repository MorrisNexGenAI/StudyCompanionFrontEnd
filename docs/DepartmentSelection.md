# 📱 USER PWA UPDATE - Department Selection Feature

## 📋 Overview
Add department selection during registration and update API to handle department-filtered content.

---

## ✅ FILES TO UPDATE

### **File 1: `src/types.ts` (or wherever your types are)**

Update the types to include department:
...
export interface PremiumProfile {
  ...
  department_name?: string | null;  // ADD THIS
...
### **File 2: `src/api/endpoints.ts`**

Add department API endpoint:
...
### **File 3: `src/components/PremiumSetup.tsx`**
...
import type { PremiumAuthResponse, PremiumProfile, Department } from '../types';
...
export const PremiumSetup = () => {
  ...
  const loadDepartments = async () => {
    try {
      const response = await apiClient.get<{ departments: Department[] }>('/premium/api/departments/');
          ...
          <CardContent sx={{ p: 4 }}>
            ...
            <Box textAlign="center" mb={4}>
              ...
              <Typography variant="body1" color="text.secondary">
                Enter your name, code, and department to access premium content
              </Typography>
            </Box>
            ...
            >
              ...
              <Typography variant="body2" color="text.secondary" mb={2}>
                Premium content is restricted to specific users. Enter your name, code, and department to access exclusive study materials.
              </Typography>
...
### **Step 1: Update Types**
Add department fields to `PremiumProfile` and `PremiumAuthResponse` types

### **Step 2: Update API Endpoints**
Add `getForRegistration` method to `departmentsAPI`

### **Step 3: Update PremiumSetup Component**
Replace entire `PremiumSetup.tsx` with the new version
...
Test:
1. Visit premium setup page
2. Verify departments dropdown loads
3. Register with department selected
4. Verify department saved in profile
...
- [ ] Update types with department fields
- [ ] Add department API endpoint
- [ ] Replace PremiumSetup component
- [ ] Test department loading
- [ ] Test registration with department
- [ ] Test registration without department
- [ ] Verify department stored in IndexedDB
...
- ✅ Department stored in local profile
- ✅ Department sent to backend for filtering
- ✅ Optional field (can skip)

**Your User PWA now supports department-based content filtering!**